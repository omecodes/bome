...
func (tx *JSONListTx) IndexInRange(after, before int64) (Cursor, int64, error) {
	var (
		total int64
		c     Cursor
	)
...
func (tx *JSONListTx) IndexBefore(index int64) (Cursor, int64, error) {
...
func (tx *JSONListTx) IndexAfter(index int64) (Cursor, int64, error) {
	o, err := tx.Client().SQLQueryFirst("select count(ind) from $table$ where ind > ?;", IntScanner, index)
	if err != nil {
		return nil, 0, err
	}
